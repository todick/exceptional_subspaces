...
if __name__ == "__main__":
    ...
    os.makedirs(f"results/RQ1/{args.method}/shuffled_real_world/{args.dataset}/{args.casestudy}", exist_ok=True)
    result_path = f"results/RQ1/{args.method}/shuffled_real_world/{args.dataset}/{args.casestudy}/{args.seed}.csv"
    with open(result_path, mode='w') as f:
        writer = csv.writer(f)
        writer.writerow(["n_rules", "sg_mean", "sg_cardinality", "sg_rule", "best_rule", "best_rule_idx", "f1", "precision", "recall", "seed", "runtime"])
    ...
    for n_rules in range(1, len(subgroups) + 1):
        first_n_groups = subgroups[:n_rules]
        f1_scores = [f1_score(seeded_groups[0], sg) for sg in first_n_groups]
        precisions = [precision_score(seeded_groups[0], sg) for sg in first_n_groups]
        recalls = [recall_score(seeded_groups[0], sg) for sg in first_n_groups]
        best_rule = np.argmax(f1_scores)
    
        with open(result_path, mode='a') as f:
            writer = csv.writer(f)
            writer.writerow([n_rules, sg_mean, sg_cardinality, seeded_rules[0], rules[best_rule], best_rule, f1_scores[best_rule], precisions[best_rule], recalls[best_rule], args.seed, time_elapsed])